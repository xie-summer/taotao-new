...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		 ...
		 * 默认情况下访问/logout将会将用户注销，包含的内容有： 1、使HttpSession失效 2、清空已配置的RememberMe验证 3、清空
		 * SecurityContextHolder 4、重定向到 /login?success
		 */
		http.logout().deleteCookies().logoutUrl("/logout").logoutSuccessUrl("/login?success").permitAll();
		/***
		 * 使用form表单登录 如果需要使用自定义登录页面需要 http.formLogin().loginPage("/login")
		 ***/
